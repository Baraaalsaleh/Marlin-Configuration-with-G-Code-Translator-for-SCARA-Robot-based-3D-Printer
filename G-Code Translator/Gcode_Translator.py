...
def find_angles(x,y,settings,file_name):
    ...
    if handed == "R":
        ...
        h = 1
    try:
        #print((x**2 + y**2 - a**2 - b**2)/(2*a*b))
        thetaB = math.acos((x**2 + y**2 - a**2 - b**2)/(2*a*b))

        thetaA = math.atan2(x,y) - math.atan2((b*math.sin(thetaB)), (a + b*math.cos(thetaB)))
                ...
                while movement > quality:
                    ...
                    if e_new != None and e_mid != None:
                        e_mid = round(e_mid + (e_new - e_mid)
                                      *quality/movement,6)
                    [thetaA,thetaB] = find_angles(x_mid - X0, y_mid - Y0,settings,gn)
                    code_line = "G1 X%f Y%f "%(thetaA, thetaB)
                    if e_mid != None:
                        code_line += "E%f "%e_mid
                    if f != None:
                        omega = find_feedrate(x_mid - X0,x_new - X0,y_mid - Y0,
                                              y_new - Y0,f,settings)
                        code_line += "F%f "%omega
                    #print("test >>>> " + str(z_new))
                    if z_new != None:
                        code_line += "Z%f "%z_new
                    
                    new_g.write(code_line.strip() + "\n")    
                    movement = math.sqrt((x_new-x_mid)**2+(y_new-y_mid)**2)
                [thetaA,thetaB] = find_angles(x_new - X0,y_new - Y0,settings,gn)
                code_line = "G1 X%f Y%f "%(thetaA, thetaB)
                if e_new != None:
                    code_line += "E%f "%e_new
                if f != None:
                    omega = find_feedrate(x_mid - X0,x_new - X0,y_mid - Y0,y_new - Y0,f,settings)
                    code_line += "F%f "%omega
            if x_new != None:
                x_old = x_new
            if y_new != None:
